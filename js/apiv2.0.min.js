...
(function() {
    ...
    Uc.getTilesUrl = function(a, b, c) {
        ...
        c = Tc[c];
        this.map.DL() && (e = 2);
        var T = (Sc[Math.abs(d + a) % Sc.length] + "?qt=tile&x=" + (d + "").replace(/-/gi, "M") + "&y=" + (a + "").replace(/-/gi, "M") + "&z=" + b + "&styles=" + c + "&scaler=" + e + (6 == x.ea.la ? "&color_dep=32&colors=50": "") + "&udt=20160401").replace(/-(\d+)/gi, "M$1");
        var sate_path = "maptile/normal"
        var level_2 = d;
        var level_3 = a;
        var level_1 = b;

        var file_path = sate_path+'/'+level_1+'/'+level_2+'/'+level_3+'.jpg';
        fs.exists(file_path, function (exists) {
          if(exists){
            /*console.log('Loading '+file_path);*/
            return file_path.replace(/-(\d+)/gi, "M$1");
          }
          else{
            mkdirSync("maptile");
            mkdirSync(sate_path);
            mkdirSync(sate_path+'/'+level_1);
            mkdirSync(sate_path+'/'+level_1+'/'+level_2);
            request.head(T, function(err, res, body){
              var tmp_file = fs.createWriteStream(file_path)
              request(T).pipe(tmp_file).on('close', function(){
                return;
    ...
    Vc.getTilesUrl = function(a, b) {
        ...
        var T = this.cO[Math.abs(c + d) % this.cO.length] + this.map.Vb + "/" + this.map.Bw + "/3/lv" + (21 - b) + "/" + c + "," + d + ".jpg"
    ...
    Wc.getTilesUrl = function(a, b) {
        var c = a.x,
        d = a.y;
        var T =(Rc[Math.abs(c + d) % Rc.length] + "u=x=" + c + ";y=" + d + ";z=" + b + ";v=009;type=sate&fm=46&udt=20141015").replace(/-(\d+)/gi, "M$1"); 
        var sate_path = "maptile/satellite"
        var level_2 = c;
        var level_3 = d;
        var level_1 = b;
    ...
    Xc.getTilesUrl = function(a, b) {
        var c = a.x,
        d = a.y;
        var hybrid_path = "maptile/hybrid";
        var cL =(Sc[Math.abs(c + d) % Sc.length] + "?qt=tile&x=" + (c + "").replace(/-/gi, "M") + "&y=" + (d + "").replace(/-/gi, "M") + "&z=" + b + "&styles=sl" + (6 == x.ea.la ? "&color_dep=32&colors=50": "") + "&udt=20141015").replace(/-(\d+)/gi, "M$1");

        var level_2 = c;
        var level_3 = d;
        var level_1 = b;
        var file_path = hybrid_path+'/'+level_1+'/'+level_2+'/'+level_3+'.jpg';

        fs.exists(file_path, function (exists) {
          if(exists){
            /*console.log('Loading hybrid'+file_path);*/
            return file_path.replace(/-(\d+)/gi, "M$1");
          }
          else{
            mkdirSync("maptile");
            mkdirSync(hybrid_path);
            mkdirSync(hybrid_path+'/'+level_1);
            mkdirSync(hybrid_path+'/'+level_1+'/'+level_2);
            request.head(cL, function(err, res, body){
              var tmp_file = fs.createWriteStream(file_path)
              request(cL).pipe(tmp_file).on('close', function(){
                return;
                /*console.log('Downloaded hybrid '+ file_path);*/
              });
            });
          }
        });
    ...
    function Ee(a, b) {
        ...
        c.refresh();
        c.ca(ve, window);
        c.ca(we); ! se && "none" != c.options.JO && (c.ca("DOMMouseScroll"), c.ca("mousewheel"));
    ...
    Ee.prototype = {
            ...
            if (! (se && 0 !== a.touches.length)) {
                ...
                b.options.up && b.options.up.call(b, a);
                    ...
                    if (b.rh) {
                        ...
                        if (se) if (b.wK && b.options.zoom) clearTimeout(b.wK),
                        b.wK = p,
        ...
        io: function(a) {
            ...
            if (b == this.x && c == this.y) {
                if (this.rh && (this.rh = q, this.options.Yx && this.options.Yx.call(this)), this.mi && this.options.Bx && ("webkit" == ae && (this.BL.style[ne] = "300ms"), this.BL.style.opacity = "0"), this.Ci && this.options.Bx)"webkit" == ae && (this.BO.style[ne] = "300ms"),
